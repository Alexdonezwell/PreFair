...
import TreeComponent2 from './TreeComponent2';

class TreeMap2 extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    findParentNode = (p, arr) => {
        const name = p.name;
        arr.push(name);
        if (p.depth == 0) {
            return arr;
        } else {
            return this.findParentNode(p.parent, arr);
        }
    }
    ...
    generate = () => {
        const { currentNode } = this.state;
        let arr = [];
        arr = this.findParentNode(currentNode, arr);
        this.props.getRoadMap(arr);
    }

    onMouseOver = (event, currentNode) => {
        this.props.hoverNode(currentNode);
    }
    ...
    render() {
        ...
        return (
            <div onClick={this.cancleActive}>
                <TreeComponent2 {...this.props} tranInfo={tranInfo} onMouseOver={this.onMouseOver} onDrop={this.onDrop} nodeClick={this.nodeClick} dataSource={dataSource} currentNode={currentNode} />
